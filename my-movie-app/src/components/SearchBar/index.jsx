    ...
    return(
        <section id="searchBar" className="flex items-center justify-center h-20  bg-background overflow-hidden">
            <form id='searchForm' className="flex items-center px-1 py-1 rounded-full border border-transparent bg-white">
                <input className="px-2 py-2 w-40 h-10 text-content-color outline-none rounded-l-full" type="text" placeholder="Search your movie" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
                <button id="searchBtn" className="flex items-center justify-center px-3 py-2 bg-white rounded-r-full border-l-0" onClick={handleSearch}>
